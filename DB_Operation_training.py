...
class db_operation:
    def __init__(self):
        ...
        self.bad_file_path='Training_raw_checking_file/bad_csv_files'
        self.good_file_path='Training_raw_checking_file/good_csv_files'
        self.log=logger()
    ...
    def data_base_connection(self,database_name):
        try:
            ...
            self.log.apply_log('Training_Logs/Genarel_log.txt', msg='database conection has been established')
        except Exception as e:
            self.log.apply_log('Training_Logs/Genarel_log.txt',msg='cannection can not be established')
            raise e
        return conn
    ...
    def create_table(self,database_name, column_name):
        try:
            ...
            cur=conn.cursor()
            cur.execute('drop table if exists good_raw_data')
            for key in column_name:
                type=column_name[key]
                key1=key.replace(' - ','_')
                try:
                    cur.execute('create table good_raw_data ({} {}(20))'.format(key1,type))
                except:
                    cur.execute('alter table good_raw_data add {} {}(10)'.format(key1,type))
            conn.commit()
            conn.close()
            self.log.apply_log('Training_Logs/Genarel_log.txt', msg='table created in database')
        except Exception as e:
            self.log.apply_log('Training_Logs/Genarel_log.txt', msg='there is a problem in creating table')
            raise e
    ...
    def replace_missing_values_with_null(self):
        try:
            ...
            self.log.apply_log('Training_Logs/Genarel_log.txt', msg='missing values replaced')
        except Exception as e:
            raise e
                ...
                try:
                    ...
                    with open(path,'r') as f:
                        ...
                        for line in enumerate(reader):
                            for l in line[1]:
                                print(l)
                                cur.execute("insert into good_raw_data values({})".format(l))
                                conn.commit()
                    self.log.apply_log('Training_Logs/Genarel_log.txt',msg='{} loded to database successfully'.format(file))
                    f.close()
                except Exception as e:
                      raise e
        except Exception as e:
    ...
    def upload_data_from_table_into_final_csv(self,database):
        ...
        try:
            ...
            self.log.apply_log('Training_Logs/Genarel_log.txt',msg='csv file exported from database to master csv file')
        except Exception as e:
            print(e)
            raise e
        finally:
